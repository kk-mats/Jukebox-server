...
export class Type {
	...
	public static async resolveTarget(
		...
		slug: string,
		target: Target
	): Promise<Failable.Type<{ targetDir: string; historiesDir: string }>> {
		...
		const targetDir = path.resolve(codes, target.targetDir || "");
		...
		return Failable.fail(FailureCode.project.invalidTarget(slug, target));
	}
	...
	public static async getBranches(
		...
		const git = simplegit(
			pathUtils.resolveChildrenPath(ownerId, r.value._id).codes
		);
		return Failable.succeed((await git.branch([])).all);
	}
